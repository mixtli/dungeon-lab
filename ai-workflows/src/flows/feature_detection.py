...
def detect_features_flow(input_data: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    """
    logger = get_run_logger()
    logger.info("Starting feature detection flow")
    ...
    try:
        ...
        send_progress_update(
            status="running", progress=10.0, message="Image loaded successfully"
        )

        # Preprocess the image
        preprocessed_img = preprocess_image(img)
        send_progress_update(
            status="running", progress=20.0, message="Image preprocessing completed"
        )
        ...
        send_progress_update(
            status="running", progress=60.0, message=f"Detected {len(doors)} doors"
        )
