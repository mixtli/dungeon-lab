...
def parse_args():
    ...
    map_gen_parser = subparsers.add_parser(
        "generate-map", help="Generate a map from a text description"
    )
    map_gen_parser.add_argument(
        "--description", "-d", required=True, help="Text description of the map"
    )
    map_gen_parser.add_argument("--user-id", "-u", required=True, help="User ID")
    map_gen_parser.add_argument(
        "--campaign-id", "-c", required=True, help="Campaign ID"
    )
    map_gen_parser.add_argument(
        "--width", "-w", type=int, default=30, help="Map width in grid cells"
    )
    map_gen_parser.add_argument(
        "--height", "-ht", type=int, default=20, help="Map height in grid cells"
    )
    ...
    map_gen_parser.add_argument(
        "--resolution", "-r", type=int, default=70, help="Pixels per grid cell"
    )

    # Feature detection command
    detect_parser = subparsers.add_parser(
        "detect-features", help="Detect features in an existing map image"
    )
    detect_parser.add_argument(
        "--image-path", "-i", required=True, help="Path to the map image"
    )
    ...
    detect_parser.add_argument(
        "--pixels-per-grid", "-p", type=int, default=70, help="Pixels per grid cell"
    )
