...
import { serverGameStateWithVirtualsSchema } from '@dungeon-lab/shared/schemas/server-game-state.schema.js';
import { GameStateModel } from '../models/game-state.model.js';
...
import { 
  ...
  isValidNextVersion,
  GameStateOperations
} from '@dungeon-lab/shared/utils/index.js';
...
export class GameStateService {
   ...
   */
  private async getGameStateByIdWithRetry(gameStateId: string, retries = 3): Promise<import('../models/game-state.model.js').IGameStateDocument | null> {
  ...
  async updateGameState(
    ...
    try {
      ...
      if (response.success && response.newVersion) {
        ...
        const { GameSessionModel } = await import('../models/game-session.model.js');
        ...
        const { SocketServer } = await import('../../../websocket/socket-server.js');
