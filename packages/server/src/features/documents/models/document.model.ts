...
import { z } from 'zod';
import { baseSchema } from '@dungeon-lab/shared/schemas/index.js';
...
export async function getDocumentModel(documentType: string) {
  switch (documentType) {
    case 'actor': {
      ...
      const { ActorDocumentModel } = await import('./actor-document.model.js');
      return ActorDocumentModel;
    }
    case 'character': {
      // Import dynamically to avoid circular dependencies
      const { CharacterDocumentModel } = await import('./character-document.model.js');
      return CharacterDocumentModel;
    }
    case 'item': {
      // Import dynamically to avoid circular dependencies
      const { ItemDocumentModel } = await import('./item-document.model.js');
      return ItemDocumentModel;
    }
    case 'vtt-document': {
      // Import dynamically to avoid circular dependencies
      const { VTTDocumentModel } = await import('./vtt-document.model.js');
