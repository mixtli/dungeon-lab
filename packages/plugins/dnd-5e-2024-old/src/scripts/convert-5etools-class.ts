...
import type {
  ...
  ISpellEntry,
  ISpellChoice
} from '../shared/types/character-class.js';
import { toLowercase, cleanRuleText } from './converter-utils.js';
import type { Ability } from '../shared/types/common.js';
import { read5eToolsData } from './import-utils.js';
...
function extractSubclassFeatures(
  ...
  for (const feature of allFeatures) {
    if (
      toLowercase(feature.className || '') !== className ||
    ...
    const hasRefFeatures =
      Array.isArray(feature.entries) &&
      feature.entries.some(
        (entry: any) => typeof entry === 'object' && entry?.type === 'refSubclassFeature'
      );
...
function normalizeSpellString(
  spellString: string | any,
  defaultTag?: string
): ISpellData | ISpellChoice {
  ...
  if (typeof spellString === 'object' && spellString !== null) {
    if (spellString.choose) {
      ...
      if (spellString.choose.fromClassList) {
        choice.classes = spellString.choose.fromClassList.map((cls: any) => toLowercase(cls.name));
      }
  ...
  return additionalSpells.map((spellEntry) => {
    ...
    if ('prepared' in spellEntry) {
      ...
      Object.entries(spellEntry.prepared).forEach(([level, spells]) => {
        ...
        result.prepared![level] = spells.map((spell) => normalizeSpellString(spell));
      });
    }
    ...
    if ('innate' in spellEntry) {
      ...
      Object.entries(spellEntry.innate).forEach(([level, spellsData]) => {
        if (Array.isArray(spellsData)) {
          ...
          result.innate![level] = spellsData.map((spell) => normalizeSpellString(spell));
          ...
          if ('resource' in spellsData) {
            ...
            Object.entries(spellsData as Record<string, any>).forEach(([tag, tagSpells]) => {
              if (Array.isArray(tagSpells)) {
                ...
                typeof tagSpells === 'object' &&
                tagSpells !== null &&
                'resource' in tagSpells
              ) {
...
export async function getSubclassesForClass(className: string, source: string): Promise<any[]> {
  try {
    ...
    if (data.subclass && Array.isArray(data.subclass)) {
      ...
      const matchingByClassField = data.subclass.filter(
        (sc: any) => sc.class === className && sc.classSource === source && sc.source === source
      );

      const matchingByClassName = data.subclass.filter(
        (sc: any) => sc.className === className && sc.classSource === source && sc.source === source
      );
      ...
      const matchingSubclasses =
        matchingByClassField.length > 0 ? matchingByClassField : matchingByClassName;
...
export function convert5eToolsClass(data: any): ICharacterClassData {
  ...
  const processedSubclasses = subclasses.map((subclass: RawSubclassData) => {
    const subclassFeaturesByLevel = extractSubclassFeatures(subclass, subclassFeatures, className);
  ...
  const result = {
    ...
    proficiencies: {
      armor: (classData.startingProficiencies?.armor || []).map((armor: string) =>
        toLowercase(armor)
      ),
      weapons: (classData.startingProficiencies?.weapons || []).map((weapon: string) =>
        toLowercase(weapon)
      ),
      tools: (classData.startingProficiencies?.tools || []).map((tool: string) =>
        toLowercase(tool)
      ),
...
export function getClassDescription(
  ...
  const classFluff = classFluffData.classFluff.find(
    (fluff: any) => fluff.name === className && fluff.source === source
  );
  ...
  const entriesContainer = classFluff.entries.find(
    (entry: any) =>
