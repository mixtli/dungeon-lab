...
import type { IBackgroundData } from '../shared/types/vttdocument.js';
...
function normalizeAbilities(abilityData: any[]): string[] {
  ...
  if (
    abilityData[0]?.choose?.weighted?.from &&
    Array.isArray(abilityData[0].choose.weighted.from)
  ) {
...
export function convert5eToolsBackground(
  ...
  if (fluffData && fluffData.backgroundFluff) {
    ...
    const fluffEntry = fluffData.backgroundFluff.find(
    ...
    if (fluffEntry && fluffEntry.entries && Array.isArray(fluffEntry.entries)) {
      ...
      description = fluffEntry.entries
        .map((entry: any) => {
  ...
  if (data.suggestedCharacteristics) {
    ...
    if (
      data.suggestedCharacteristics.personalityTrait &&
      Array.isArray(data.suggestedCharacteristics.personalityTrait)
    ) {
