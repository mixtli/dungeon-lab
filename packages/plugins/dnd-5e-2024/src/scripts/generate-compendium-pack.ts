...
import { CompendiumPackGenerator, GeneratorOptions } from '../5etools-converter/generator/compendium-pack-generator.js';
...
async function main() {
  const { values } = parseArgs({
    ...
    options: {
      ...
      'content-types': {
        type: 'string',
        default: 'monsters,spells,backgrounds,items,classes,species,feats,conditions,actions,rules,languages,senses'
      },
      'include-assets': {
        type: 'boolean',
        default: true
...
Options:
  ...
  --content-types <types>      Comma-separated content types (default: monsters,spells,backgrounds,items,classes,species,feats,conditions,actions,rules,languages,senses)
  --include-assets             Include image assets (default: true)
  --skip-missing-assets        Skip missing assets instead of warning (default: false)
  -h, --help                   Show help

Available content types: monsters, spells, backgrounds, items, classes, species, feats, conditions, actions, rules, languages, senses
...
`);
  ...
  const options: GeneratorOptions = {
    ...
    contentTypes: (values['content-types'] ?? 'monsters,spells,backgrounds,items,classes,species,feats,conditions,actions,rules,languages,senses').split(',').map(s => s.trim()),
