...
export function convert5eToolsMonster(monsterData: any, fluffData?: any): { monster: IMonster; imagePath?: string } {
    ...
    const monster: IMonster = {
        ...
        type: toLowercase(monsterData.type || ''),
        alignment: toLowercase(Array.isArray(monsterData.alignment) ? monsterData.alignment.join(' ') : (monsterData.alignment || '')),
        armorClass: {
            value: Array.isArray(monsterData.ac) ? (monsterData.ac[0] || 10) : (monsterData.ac || 10),
            type: Array.isArray(monsterData.ac) && monsterData.ac[1] ? toLowercase(monsterData.ac[1]) : undefined
        },
        hitPoints: {
