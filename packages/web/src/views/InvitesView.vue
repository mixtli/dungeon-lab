<template>
  <div class="container mx-auto px-4 py-8">
    ...
    <div v-if="showCharacterSelect" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4">
      <div class="bg-white rounded-lg shadow-xl w-full max-w-lg">
        <div class="p-6">
          ...
          <div v-else-if="!compatibleCharacters.length" class="text-center py-8">
            <p class="mb-4">You don't have any characters compatible with this campaign's game system.</p>
            <button 
              class="px-4 py-2 bg-primary-600 text-white rounded hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-opacity-50"
          ...
          <div v-else class="grid gap-4 max-h-96 overflow-y-auto">
            <div v-for="character in compatibleCharacters" :key="character.id" 
...
async function loadCompatibleCharacters(gameSystemId: string) {
  ...
  try {
    ...
    compatibleCharacters.value = actorStore.actors.filter(
      (actor: IActor) => actor.gameSystemId === gameSystemId && actor.createdBy === authStore.user?.id
    );
  } catch (err: any) {
