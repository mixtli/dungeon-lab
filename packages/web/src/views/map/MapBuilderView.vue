...
import { useWorkflowProgress } from '../../composables/useWorkflowProgress.js';
import { useSocketStore } from '../../stores/socket.store.ts';
import type { MapGenerationResponse, MapEditResponse, MapFeatureDetectionResponse } from '@dungeon-lab/shared/types/socket/index.js';
import axios from 'axios';
import { MapsClient } from '@dungeon-lab/client/maps.client.js';
import { convertUVTTToMapData } from '@dungeon-lab/shared/utils/uvtt-converter.js';
      ...
      if (payload.state === 'COMPLETED' || payload.state === 'Completed') {
        ...
        if (payload.result && typeof payload.result === 'object') {
          ...
          if (payload.result.uvtt_file_url && typeof payload.result.uvtt_file_url === 'string') {
            uvttUrl.value = transformAssetUrl(payload.result.uvtt_file_url);
...
const proceedToEdit = async () => {
  ...
  try {
    ...
    const imageResponse = await fetch(transformAssetUrl(previewImage.value));
      ...
      <div v-if="previewReady" class="space-y-6">
        <div class="bg-stone dark:bg-stone-700 rounded-lg shadow-xl border border-stone-300 dark:border-stone-600 p-4">
          ...
          <div class="relative rounded-lg overflow-hidden">
            <img :src="transformAssetUrl(previewImage)" :alt="parameters.name" class="w-full h-auto" />
          </div>
