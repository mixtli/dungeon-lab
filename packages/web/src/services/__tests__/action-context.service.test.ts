...
import { ActionContextImpl, createActionContext } from '../action-context.service.ts';
import type { RollServerResult } from '@dungeon-lab/shared/schemas/roll.schema.js';
import type { RollData, RollRequestSpec } from '@dungeon-lab/shared-ui/types/action-context.js';
...
describe('ActionContextImpl', () => {
  ...
  beforeEach(async () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { useChatStore } = await vi.importMock('../stores/chat.store.ts') as any;
