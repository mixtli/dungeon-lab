...
export class GMActionHandlerService {
  ...
  /**
   * Handle document update requests - requires GM approval
   */
  private async handleDocumentUpdate(request: GameActionRequest) {
    const params = request.parameters as UpdateDocumentParameters;
    
    console.log('[GMActionHandler] Processing document update:', {
    ...
    try {
      ...
      // Check if action requires approval (should be true for update-document)
      if (requiresApproval(request.action)) {
        await this.sendApprovalRequest(request, params.operations);
      } else {
        // Auto-approve case (shouldn't happen for document updates, but included for completeness)
        console.log('[GMActionHandler] Auto-approving document update');
        await this.executeDocumentUpdate(params.operations, request.id);
      }

    } catch (error) {
      console.error('[GMActionHandler] Error processing document update:', error);
      this.socketStore.emit('gameAction:response', {
        success: false,
        requestId: request.id,
        error: {
  ...
  private async sendApprovalRequest(request: GameActionRequest, operations: StateOperation[]) {
    ...
    console.log('[GMActionHandler] Approval request sent to chat:', {
      requestId: request.id,
      actionType: request.action,
      playerName
    });
  ...
  private async executeDocumentUpdate(operations: StateOperation[], requestId: string) {
    try {
      ...
      // Use game state store to apply operations
      const updateResult = await this.gameStateStore.updateGameState(operations);
      
      if (updateResult.success) {
        console.log('[GMActionHandler] Document update approved and executed:', { requestId });
      ...
      console.error('[GMActionHandler] Error executing document update:', error);
      this.socketStore.emit('gameAction:response', {
        success: false,
        requestId: requestId,
        error: {
          code: 'EXECUTION_ERROR',
          message: 'Failed to execute document update'
        }
      });
    }
  }
